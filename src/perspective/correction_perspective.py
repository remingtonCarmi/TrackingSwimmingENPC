...
def correct_perspective_img(img, srce, dst, testing=False, display=False):
    """
    ...
    Args:
        ...
        srce (list of four tuples): the four points selected on the input image to correct perspective
    ...
    m = cv2.getPerspectiveTransform(srce, dst)
    ...
    if testing:
        if display:
            ...
            ax1.imshow(img)
            x = [srce[0][0], srce[2][0], srce[3][0], srce[1][0], srce[0][0]]
            y = [srce[0][1], srce[2][1], srce[3][1], srce[1][1], srce[0][1]]
...
if __name__ == "__main__":
    vid0 = Path("../../data/videos/vid0")
    list_images = extract_image_video(str(vid0), 4, 5, False)
    IMAGE_PATH = Path("../../test/imageTest1.jpg")
    cv2.imwrite(str(IMAGE_PATH), list_images[0])
