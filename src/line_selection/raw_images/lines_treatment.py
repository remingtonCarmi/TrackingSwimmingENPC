...
def crop_image(image, pourcent_x, pourcent_length, pourcent_y, pourcent_thickness):
    ...
    Args:
        ...
        pourcent_x (float): the returned image will start at pourcent_x * height
            in the original image on the first axis

        pourcent_length (float): the returned image height will be pourcent_length * height

        pourcent_y (float): the returned image will start at pourcent_y * width
            in the original image on the second axis
    ...
    return image[x_start: x_start + int(pourcent_length * height), y_start: y_start + int(pourcent_thickness * width)]


# ---- To compute the canny edges ---- #
...
# ---- To extract the lines from pixels ---- #
...
# ---- To purify the lines using the color ---- #


# ---- To expand the lines using the color ---- #


# ---- To join the lines with the ends ---- #
...
def join_ends(lines, interesting_lines, end_lines):
    """
    ...
    Args:
        ...
        end_lines (array, shape = (len(nb_line), 2, 2)): the list of
            the extremum points of each line
    """
    nb_lines = len(lines)
    ...
    for index_line in range(nb_lines):
        if interesting_lines[index_line]:
            ...
            # If two lines are closed, we add the second line
            # in the first and turn the second one useless
            if min(distance) < JOIN_DISTANCE:
                ...
                return join_ends(lines, interesting_lines, end_lines)
...
# ---- To impose the slope of the lines that have a small accuracy ---- #
def impose_slope(lines, info_reg, interesting_lines):
    """
    Imposes the slope to the lines that have a too low accuracy.
    ...
    for index_line in range(nb_lines):
        if interesting_lines[index_line] and not good_lines[index_line]:
            line = np.array(lines[index_line])
            # We impose the slope and compute the intercept of the linear regression
            new_intercept = np.mean(line[:, 0]) - mean_coeff * np.mean(line[:, 1])

            # We compute the accuracy : || y - y_reg ||^2 = Var(y)*(1 - R^2)
            y_reg = np.array([mean_coeff * point[1] + new_intercept for point in line])
            diff = alg.norm(line[:, 1] - y_reg)
            var_y = np.std(line[:, 1]) ** 2
            new_accuracy = 1 - diff ** 2 / var_y
            info_reg[index_line] = [mean_coeff, new_intercept, new_accuracy]


# ---- To join the lines that have the same area ---- #
def integral_difference(info_reg, index_ref, interesting_lines, w_dim):
    """
    Computes the difference of the integral of line[index_ref] and all the other lines.

    Args:

        info_reg (list of lists of 3 elements): (slope, intercept, accuracy) of each line.

        index_ref (int): the line that we are interested in.

        interesting_lines (array of size = len(lines)):
            saved_lines[i] = 1 if the line i is interesting, = 0 if not.

        w_dim (int): the width of the original image.

    Returns:
        differences (array, shape = (number of interesting lines)): the integral differences.
    """
    ...
    for index_line in range(nb_lines):
        if interesting_lines[index_line]:
            if index_line == index_ref:
                ...
                differences[nb_interesting] = abs(w_dim * (w_dim * (slope - slope_ref) / 2 + intercept - intercept_ref))
            nb_interesting += 1

    return differences


def join_area(lines, info_reg, interesting_lines, width):
    """
    ...
    for index_line in range(nb_lines):
        if interesting_lines[index_line]:
            ...
            if min(distance) < DTSTANCE_INTEGRAL:
                ...
                return join_area(lines, info_reg, interesting_lines, width)


# ---- To purify the linear regression by erasing the lines that have an extreme slope. ---- #
    ...
    for index_line in range(nb_lines):
        if interesting_lines[index_line]:
            ...
            for index_points in range(nb_points):
                (x_coord, y_coord) = lines[index_line][index_points]
                (red, green, blue) = image[x_coord, y_coord]
                sum_r[index_points] = red
                sum_g[index_points] = green
                sum_b[index_points] = blue
...
# ---- To update the regression information, the lines and the interesting lines ---- #
def update_lines(lines, info_reg, interesting_lines):
    ...
    Returns:
        ...
        new_info_reg (list of lists of 3 elements): (slope, intercept, accuracy)
            of each updated line.
...
# ---- To show the results ---- #
def show_lines(lines, h_dim, w_dim, channel):
    """
    ...
    Args:
        ...
        h_dim (int): the height of the original image.
    ...
    """
    image_lines = np.zeros((h_dim, w_dim, channel), np.uint8)
    for line in lines:
...
def show_reg(lines, image, info_reg, with_image=True):
    """
    ...
    if with_image:
        image_reg = image.copy()
    else:
        image_reg = np.zeros((h_dim, w_dim, nb_channel), np.uint8)

    for index_line in range(nb_lines):
        (coeff, intercept) = info_reg[index_line][: 2]
        color = rd.randint(0, 25, 3, np.uint8)
        for y_coord in range(w_dim):
            if in_image(int(y_coord * coeff + intercept), y_coord, h_dim, w_dim):
                image_reg[int(y_coord * coeff + intercept), y_coord] = color
...
def select_line(image):
    ...
    join_ends(list_lines, saved_lines, lines_ends)
    ...
    # We impose the slope of the lines that have too low accuracy.
    impose_slope(list_lines, reg_info, saved_lines)

    # Joins the lines that are closed according to their linear regressions.
    join_area(list_lines, reg_info, saved_lines, w_dim)
